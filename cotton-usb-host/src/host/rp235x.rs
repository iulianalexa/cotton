...
impl Rp235xHostController {
    ...
    async fn send_setup(
        ...
        self.regs
            ...
            .modify(|_, w| {
                if needs_pream {
                    w.preamble_en().set_bit()
                } else {
                    w.preamble_en().clear_bit()
                }
            });
...
impl HostController for Rp235xHostController {
    ...
    async fn control_transfer<'a>(
        ...
        self.send_setup(address, &setup, needs_pream).await?;
        match data_phase {
            DataPhase::In(buf) => {
                let sz = self
                    .control_transfer_in(
                    ...
                    .await?;
                self.control_transfer_out(address, packet_size, 0, needs_pream, &[])
                    .await?;
                Ok(sz)
            }
            DataPhase::Out(buf) => {
                let sz = self
                    ...
                    .await?;
                self.control_transfer_in(address, packet_size, 0, needs_pream, &mut [])
                    .await?;
                Ok(sz)
            }
            DataPhase::None => {
